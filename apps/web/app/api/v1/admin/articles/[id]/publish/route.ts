...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  if (!rateLimit(`admin:article:publish:${auth.user.id}`, 60)) return fail(429, "RATE_LIMIT_EXCEEDED", "Çok fazla istek");

  try {
    const updated = await prisma.article.update({
      where: { id: params.id },
      data: { status: "published", publicationStatus: "published", publishedAt: new Date() }
    });
    return ok(updated);
  } catch {
